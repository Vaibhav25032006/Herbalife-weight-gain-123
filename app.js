...
// Custom Component for CDN Lucide Icons 
const LucideIcon = ({ name, size = 24, className = "" }) => {
    useEffect(() => {
        if (window.lucide) window.lucide.createIcons();
    }, [name]);
    return <i data-lucide={name.toLowerCase()} style={{ width: size, height: size, display: 'inline-block' }} className={className}></i>;
};
...
function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = auth.onAuthStateChanged(async (u) => {
      setUser(u);
      if (u) {
        const profileDoc = db.collection('artifacts').doc(appId).collection('users').doc(u.uid).collection('profile').doc('data');
  ...
  useEffect(() => {
    ...
    const trackingDoc = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('tracking').doc('data');
    
    const unsubscribe = trackingDoc.onSnapshot((snap) => {
      if (snap.exists) {
  ...
  const handleGlobalVerification = async () => {
    if (!user) return;
    try {
      const profileDoc = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('profile').doc('data');
      await profileDoc.set({ cameraVerified: true, verifiedAt: new Date().toISOString() }, { merge: true });
      setIsVerified(true);
    } catch (e) {
  ...
  const completeTask = async (taskId) => {
    ...
    const trackingDoc = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('tracking').doc('data');
    await trackingDoc.set({ history: newHistory }, { merge: true });
  };
  ...
  const analyzeAndComplete = async () => {
    ...
    try {
      ...
      const prompt = `Task: Verify if user is consuming ${currentTask.title} (${currentTask.detail}). User says: "${userNote}" Rules: If the image shows any food, drink, or health product, say yes. Be lenient. Return JSON: {"verified": true, "reason": "Verified"}`;
    ...
    } catch (error) { speak("Network check karein."); }
  ...
  const chartData = useMemo(() => {
    ...
    return Array.from({ length: numDays }).map((_, i) => {
      ...
      return { day: day, tasks: history[dateStr] ? Object.keys(history[dateStr]).length : 0 };
    });
  }, [history, viewMonth]);
  ...
  if (!user) return (
    <div className="h-screen bg-white flex flex-col items-center justify-center p-10 text-center">
      <div className="w-20 h-20 bg-emerald-500 rounded-[30px] flex items-center justify-center mb-8 shadow-xl"><LucideIcon name="Zap" size={40} className="text-white fill-white" /></div>
      <h1 className="text-3xl font-black mb-6 italic tracking-tighter text-slate-900">HERBALIFE AI</h1>
      <button onClick={() => auth.signInWithPopup(new firebase.auth.GoogleAuthProvider())} className="w-full bg-slate-900 text-white py-4 rounded-[25px] font-black flex items-center justify-center gap-3 shadow-lg"><LucideIcon name="LogIn" size={18} />LOGIN WITH GOOGLE</button>
    </div>
  );

  if (!permissionsGranted) return (
    <div className="h-screen bg-white flex flex-col items-center justify-center relative overflow-hidden p-10 text-center">
      <LucideIcon name="ShieldCheck" size={64} className="text-emerald-500 mx-auto mb-6" />
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f8fafc] max-w-md mx-auto relative overflow-hidden font-sans shadow-xl">
        ...
        <div className="flex justify-between items-center">
          <div className="relative">
            <button onClick={() => setShowAccountMenu(!showAccountMenu)} className="flex items-center gap-2 bg-black/10 p-1.5 pr-3 rounded-full">
              {user.photoURL ? <img src={user.photoURL} className="w-7 h-7 rounded-full" /> : <LucideIcon name="User" size={14}/>}
            ...
            {showAccountMenu && (
              <div className="absolute top-12 left-0 w-44 bg-white rounded-2xl shadow-2xl z-50 p-1 text-slate-800 border border-slate-100">
                <button onClick={() => auth.signOut()} className="w-full flex items-center gap-3 p-3 hover:bg-red-50 rounded-xl text-red-600 font-bold text-xs"><LucideIcon name="LogOut" size={14}/> Sign Out</button>
              </div>
            )}
          </div>
              ...
              return (
                ...
                >
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isDone ? 'bg-emerald-500 text-white' : isLocked ? 'bg-slate-200 text-slate-400' : 'bg-slate-100 text-slate-400'}`}>
                    {isLocked ? <LucideIcon name="Lock" size={18} /> : <LucideIcon name={item.icon} size={18} />}
                  </div>
                  ...
                  <div className={`w-8 h-8 rounded-full border flex items-center justify-center ${isDone ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-slate-100'}`}>
                    {isDone ? <LucideIcon name="CheckCircle2" size={16} /> : isLocked ? <div className="w-1 h-1 rounded-full bg-slate-200" /> : <div className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />}
                  </div>
                </div>
              );
            })}
          ...
          <div className="space-y-6">
            <div className="bg-white p-5 rounded-[30px] shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <button onClick={() => setViewMonth(new Date(viewMonth.getFullYear(), viewMonth.getMonth() - 1, 1))}><LucideIcon name="ChevronLeft" size={18}/></button>
                ...
                <button onClick={() => setViewMonth(new Date(viewMonth.getFullYear(), viewMonth.getMonth() + 1, 1))}><LucideIcon name="ChevronRight" size={18}/></button>
              </div>
                ...
                {schedule.map(item => {
                  ...
                  return (
                    <div key={item.id} className={`flex items-center gap-3 p-3 rounded-2xl ${isCompleted ? 'bg-emerald-50' : 'bg-slate-50 opacity-50'}`}>
                      ...
                      {isCompleted ? <LucideIcon name="CheckCircle2" size={14} className="text-emerald-500" /> : <LucideIcon name="Circle" size={14} className="text-slate-200" />}
                    </div>
                  );
                })}
              </div>
            ...
            <div className="h-48 bg-white p-5 rounded-[30px] shadow-sm">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  ...
                  <Tooltip contentStyle={{ borderRadius: '15px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', fontSize: '10px', fontWeight: 'bold' }} labelFormatter={(val) => `Day ${val}`} />
        ...
        <div className="absolute inset-0 z-[100] bg-slate-900/70 backdrop-blur-md flex items-center justify-center p-8">
          <div className="bg-white w-full rounded-[40px] p-8 text-center shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-4">
              <button onClick={() => { setFacingMode('user'); setShowCameraMode(false); setCameraActive(true); }} className="p-6 bg-slate-50 rounded-[30px] flex flex-col items-center gap-3 transition-transform active:scale-95"><LucideIcon name="User" size={24} className="text-emerald-600" /><span className="font-black text-[9px]">Selfie</span></button>
              <button onClick={() => { setFacingMode('environment'); setShowCameraMode(false); setCameraActive(true); }} className="p-6 bg-slate-50 rounded-[30px] flex flex-col items-center gap-3 transition-transform active:scale-95"><LucideIcon name="FlipHorizontal" size={24} className="text-emerald-600" /><span className="font-black text-[9px]">Product</span></button>
            </div>
        ...
        <div className="absolute inset-0 z-[110] bg-black">
          ...
          <div className="absolute bottom-8 left-6 right-6 flex flex-col gap-3">
            <div className="bg-white/10 backdrop-blur-2xl border border-white/20 rounded-[30px] p-3 flex items-center gap-3">
              <input type="text" placeholder="Add note..." value={userNote} onChange={(e) => setUserNote(e.target.value)} className="bg-transparent border-none outline-none text-white text-sm flex-1 font-bold px-2 placeholder:text-white/30" />
              <button onClick={() => { if(isListening) { recognitionRef.current.stop(); setIsListening(false); } else { recognitionRef.current.start(); setIsListening(true); } }} className={`${isListening ? 'bg-red-500' : 'bg-emerald-500'} p-3 rounded-2xl text-white transition-all`}>
                {isListening ? <LucideIcon name="MicOff" size={18} /> : <LucideIcon name="Mic" size={18} />}
              </button>
            </div>
            
            <button onClick={analyzeAndComplete} disabled={analyzing} className="w-full bg-white text-black h-16 rounded-[30px] font-black shadow-2xl flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50">
              {analyzing ? <div className="w-5 h-5 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin" /> : <><LucideIcon name="Zap" size={20} className="fill-black"/>AI SCAN KARO</>}
            </button>
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/90 backdrop-blur-2xl p-4 px-10 flex justify-between items-center rounded-t-[40px] shadow-2xl z-20">
        <button onClick={() => setActiveTab('today')} className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'today' ? 'text-emerald-600 scale-110' : 'text-slate-300'}`}><LucideIcon name="Clock" size={24} /><span className="text-[8px] font-black uppercase">Today</span></button>
        <button onClick={() => setActiveTab('stats')} className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'stats' ? 'text-emerald-600 scale-110' : 'text-slate-300'}`}><LucideIcon name="Calendar" size={24} /><span className="text-[8px] font-black uppercase">Archive</span></button>
        <a href="https://herbalife-smart-wellness-studio.my.canva.site/page-3" target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-1 text-slate-300 hover:text-emerald-600 transition-all">
